...
# Explanation:
# In the 0th query, the subarray is [4,6,5]. This can be rearranged as
# [6,5,4], which is an arithmetic sequence. In the 1st query, the subarray is
# [4,6,5,9]. This cannot be rearranged as an arithmetic sequence. In the 2nd
# query, the subarray is [5,9,3,7]. This can be rearranged as [3,5,7,9],
...
class Solution:
    def isArithmeticSequenceNaive(self, nums: List[int]) -> bool:
        '''
        O(mnlogn) time - Brute force but works
        '''
        nums.sort()
        if len(nums) < 2:
            return True
        ...
        while ptr < len(nums):
            if nums[ptr] - nums[ptr - 1] != difference:
                return False
            ptr += 1

        return True

    def isArithmeticSequence(self, nums: List[int]) -> bool:
        '''
        O(mn) time - exploits the symmetry of the AP
        '''
        min_val, max_val = min(nums), max(nums)

        if (max_val - min_val) % (len(nums) - 1):
            return False
        diff = int((max_val - min_val) / (len(nums) - 1))
        nums = set(nums)
        incr = 1

        while min_val + incr * diff < max_val:
            if min_val + incr * diff not in nums:
                return False
            incr += 1
    ...
    def checkArithmeticSubarrays(self, nums: List[int], l: List[int], r: List[int]) -> List[bool]:
        ...
        while ptr < m:
            answer.append(self.isArithmeticSequence(nums[l[ptr]:r[ptr] + 1]))
            ptr += 1
