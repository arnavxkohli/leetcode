...
int garbageCollection(char** garbage, int garbageSize, int* travel, int travelSize) {
    int houseCount = 0;
    // For each type, you need to find the last house needed to be visited
    int lastHouse[] = {0, 0, 0};
    // Keep track of the global garbage amount
    int totalGarbage = 0;

    while(houseCount < garbageSize){
        char* collect = garbage[houseCount];
        int collectSize = strlen(collect);
        for(int i = 0; i < collectSize; i++){
            switch (collect[i]) {
                case 'M':
                    totalGarbage += 1;
                    lastHouse[0] = houseCount; // Incrementing is a mistake btw.
                    break;
                case 'G':
                    totalGarbage += 1;
                    lastHouse[1] = houseCount;
                    break;
                case 'P':
                    totalGarbage += 1;
                    lastHouse[2] = houseCount;
                    break;
                default:
                    break;
            }
    ...
    for(int i = 0; i < 3; i++){
        for(int j = 0; j < lastHouse[i]; j++){
            totalGarbage += travel[j];
        }
    }
    return totalGarbage;
}
